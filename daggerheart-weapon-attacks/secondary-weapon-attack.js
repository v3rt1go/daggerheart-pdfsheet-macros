/**
 * Daggerheart Secondary Weapon Attack Macro
 * For use with PDF-Pager label mapping
 * Maps to "Secondary Weapon" label in character sheet
 */
  ...
  const enhancedFlavor = `
    <div style="border: 3px solid ${resultColor}; border-radius: 10px; padding: 20px; margin: 10px 0; background: linear-gradient(135deg, rgba(255,255,255,0.1), rgba(0,0,0,0.1)); box-shadow: 0 4px 8px rgba(0,0,0,0.3);">
        <div style="text-align: center; font-size: 20px; font-weight: bold; color: ${resultColor}; margin-bottom: 15px; text-shadow: 2px 2px 4px rgba(0,0,0,0.7);">
            🗡️ ${weaponName.toUpperCase() || "SECONDARY WEAPON"} ATTACK 🗡️
        </div>
        ...
        <div style="background: rgba(255,255,255,0.1); border-radius: 6px; padding: 10px; margin: 10px 0;">
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 8px; font-size: 12px; line-height: 1.3;">
                <div><strong>🗺</strong> ${weaponName || "Unnamed"}</div>
...
(async () => {
  ...
  if (actor) {
    weaponName =
      ui.pdfpager.getPDFValue(actor, "doc_0_Secondary Weapon Name") || "";
    weaponTrait =
      ui.pdfpager.getPDFValue(actor, "doc_0_Secondary Weapon Trait") ||
      "Agility";
    weaponRange =
      ui.pdfpager.getPDFValue(actor, "doc_0_Secondary Weapon Range") || "Melee";
    damageFormula =
      ui.pdfpager.getPDFValue(actor, "doc_0_Secondary Weapon Damage Dice") ||
      "";
    damageType =
      ui.pdfpager.getPDFValue(actor, "doc_0_Secondary Weapon Damage Type") ||
      "phy";
    weaponFeature =
      ui.pdfpager.getPDFValue(actor, "doc_0_Secondary Weapon Feature") || "";
  ...
  new foundry.applications.api.DialogV2({
    window: {
      title: "🗡️ Secondary Weapon Attack 🗡️",
      width: 750,
      height: "auto",
      resizable: true,
    },
    content: `
        <style>
            ...
            .die-toggle:checked + label {
                ...
                border-color: #5dade2;
                color: white;
                box-shadow: 0 0 25px rgba(52, 152, 219, 0.7), 0 6px 12px rgba(0,0,0,0.4);
        ...
        <div class="dh-container">
            <div class="dh-title">🗡️ ${
              weaponName.toUpperCase() || "SECONDARY WEAPON"
            } ATTACK 🗡️</div>
